...
static u8g2_uint_t u8g2_draw_string_khmer(u8g2_t *u8g2, u8g2_uint_t x,
  ...
  while (*(in_ptr) != 0) {
    ...
    int g = *(in_ptr++);
    if (g < 255 || g > 0xe200 || CAT(g) == CAT_OTHER) {
      *(out_ptr++) = g;
      // printf("0x%x\n", g);
      // printf("\n");
      continue;
    ...
    for (int i = 1; i < sizeof(buf) / sizeof(*buf); ++i) {
      ...
      if (buf[i].encoding == 0x17B6 || buf[i].encoding == 0x17C4 ||
          buf[i].encoding == 0x17C5)
        has_aa = 1;
    }
    ...
    for (int i = 0; i < sizeof(buf) / sizeof(*buf); ++i) {
      if (buf[i].encoding > 0) {
        ...
        if (buf[i].charcat == CAT_COENG_RO && has_coeng && !has_coeng_post) {
          if (buf[i - 2].encoding != 0x179F && buf[i - 2].encoding != 0x1799 &&
              buf[i - 1].encoding != 0x178D && buf[i - 1].encoding != 0x178E &&
              buf[i - 2].encoding != 0x179B && buf[i - 2].encoding != 0x17A0 &&
              buf[i - 1].encoding != 0x1783 && buf[i - 1].encoding != 0x1788 &&
              buf[i - 2].encoding != 0x1789) {
            buf[i].encoding =
                GLYPH_GET_BASE_SUB2(GLYPH_GET_BASE(buf[i].encoding));
          }
        } else if (buf[i].charcat == CAT_COENG &&
                   buf[i - 1].charcat == CAT_BASE) {
        }

        if (buf[i].charcat == CAT_COENG_POST &&
            buf[i - 1].charcat == CAT_COENG) {
          buf[i].encoding =
              GLYPH_GET_BASE_SUB2(GLYPH_GET_BASE(buf[i].encoding));
        }
        ...
        if (buf[i].encoding == 0x179A || buf[i].encoding == 0x179C) {
          if (buf[i + 1].charcat == CAT_VA || buf[i + 1].encoding == 0x17C6 ||
              buf[i + 1].encoding == 0x17C9) {
    ...
    // printf("before sort: "); log_unicode_array(buf,
    ...
    for (int i = 0; i < sizeof(buf) / sizeof(*buf); ++i) {
      if (buf[i].encoding > 0) {
        ...
        if (buf[i].encoding == 0x1789) {
          if (buf[i + 1].charcat == CAT_COENG ||
              buf[i + 1].charcat == CAT_COENG_POST) {
...
u8g2_uint_t u8g2_DrawKhmer(u8g2_t *u8g2, u8g2_uint_t x, u8g2_uint_t y,
                           const char *str) {
